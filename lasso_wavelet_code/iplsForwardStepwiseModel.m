function [model] = iplsForwardStepwiseModel(X, Y, index_mccv, nInter, scaling_method)
% 23.11.2023
...
for i=1:nInter,
    ...
    if sum(can_enter)>=1,
        ...
        steps_log=[steps_log; indexSelect];
    else
        ind_add=0;
    end
    ...
    % interval can be removed from the the model if RMSE_ref > RMSE_tent(:,k):
    % that is, if H0 is rejected:
    ...
    % stop if it no interval was added or removed
    % OR if it removed the last interval that entered the model
    if ind_remove==ind_add,
        break
    end
